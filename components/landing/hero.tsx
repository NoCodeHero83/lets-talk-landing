...
export function Hero() {
  ...
  return (
    <section className="relative flex min-h-[80vh] w-full flex-col items-center justify-center overflow-hidden rounded-md bg-background pt-20 sm:pt-28 pb-16 sm:pb-24">
      {/* Lamp effect - from components/ui/hero.tsx prompt */}
      <div className="absolute top-0 isolate z-0 flex w-screen flex-1 items-start justify-center pointer-events-none">
        ...
        {/* Right gradient cone - fixed for Tailwind v4 */}
        <motion.div
          initial={{ opacity: 0.5, width: "15rem" }}
          whileInView={{ opacity: 1, width: "30rem" }}
          transition={{
      ...
      <motion.div
        ...
        ref={ref}
        className={`relative z-50 container flex justify-center flex-1 flex-col px-5 md:px-10 gap-4 transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
      >
